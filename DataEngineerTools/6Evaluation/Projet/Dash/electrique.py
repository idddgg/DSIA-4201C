...
cars = [car for car in get_all_cars() if car.get("fuel_type", "").lower() == "électrique"]

def create_electrique_layout():
    """Retourne le layout de la page électrique avec 3 images dans le carrousel."""

    if not cars:
        return html.Div(
            "Aucune voiture électrique trouvée.",
    ...
    return html.Div(
        children=[
            html.H1(
                "BMW - Voitures électriques",
                style={"textAlign": "center", "marginBottom": "20px", "color": "#0078d7", "fontSize": "32px"},
            ),
            dcc.Store(id="electrique-current-image-index", data=0),
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.Div(
                        id="electrique-images-container",
                        style={
                            "display": "flex",
...
def register_electrique_callbacks(app):
    """Enregistre les callbacks de la page électrique."""

    @app.callback(
        [Output("electrique-images-container", "children"), Output("electrique-current-image-index", "data")],
        [Input("electrique-prev-button", "n_clicks"), Input("electrique-next-button", "n_clicks")],
        State("electrique-current-image-index", "data"),
    )
    def update_images(prev_clicks, next_clicks, current_index):
        ...
        if "electrique-prev-button" in changed_id:
            current_index = (current_index - 1) % total_images  # Boucle circulaire vers l'arrière
        elif "electrique-next-button" in changed_id:
