...
# Charger les voitures essence
cars = [car for car in get_all_cars() if car.get("fuel_type", "").lower() == "essence"]

def create_essence_layout():
    """Retourne le layout de la page essence avec 3 images dans le carrousel."""
    if not cars:
        return html.Div(
            "Aucune voiture essence trouvée.",
    ...
    return html.Div(
        children=[
            html.H1(
                "BMW - Voitures Essence",
                style={"textAlign": "center", "marginBottom": "20px", "color": "#0078d7", "fontSize": "32px"},
            ),
            dcc.Store(id="essence-current-image-index", data=0),  # Stocke l'indice de l'image courante
...
def register_essence_callbacks(app):
    """Enregistre les callbacks de la page essence."""

    @app.callback(
        [Output("essence-images-container", "children"), Output("essence-current-image-index", "data")],
        [Input("essence-prev-button", "n_clicks"), Input("essence-next-button", "n_clicks")],
        State("essence-current-image-index", "data"),
    )
